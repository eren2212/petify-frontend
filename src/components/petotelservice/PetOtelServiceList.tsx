...
import AddPetOtelServiceModal from "./AddPetOtelServiceModal";
import { COLORS } from "../../styles/theme/color";
import {
  useMyPetOtelServices,
  useDeletePetOtelService,
  usePetOtelServiceCategories,
  useUpdatePetOtelServiceStatus,
} from "../../hooks";
...
export default function PetOtelServiceList() {
  ...
  // Fetch data
  const { data, isLoading, refetch, isRefetching } = useMyPetOtelServices();
  const { data: categories = [] } = usePetOtelServiceCategories();
  ...
  const filteredServices = services.filter((service) => {
    if (selectedCategory && service.service_category_id !== selectedCategory) {
      return false;
    }
    if (selectedStatus !== undefined && service.is_active !== selectedStatus) {
      return false;
    }
    return true;
  });
  ...
  const handleDeleteService = (service: Service) => {
    Alert.alert(
      "Hizmeti Sil",
      `"${service.pet_hotel_service_categories.name_tr}" hizmetini silmek istediğinizden emin misiniz?`,
      [
        {
          text: "İptal",
          style: "cancel",
  ...
  const handleToggleStatus = (serviceId: string, currentStatus: boolean) => {
    updateStatus(
      { id: serviceId, status: !currentStatus },
      {
        onSuccess: () => {
          Toast.show({
            type: "success",
            text1: `Hizmet ${!currentStatus ? "aktif" : "pasif"} edildi!`,
            bottomOffset: 40,
          });
        },
        onError: (error: any) => {
          Toast.show({
            ...
            text1:
              error?.response?.data?.message ||
              "Hizmet durumu güncellenirken bir hata oluştu",
            bottomOffset: 40,
          });
        },
      }
        ...
        >
          <View className="px-6 py-4">
            <Text className="text-lg font-bold text-gray-900 mb-4">
              Hizmetleri Yönet ({filteredServices.length})
            </Text>
            ...
            {filteredServices.map((service: Service) => (
              ...
              >
                ...
                {/* Status Toggle */}
                <View className="mb-3 flex-row items-center justify-between bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                  <View className="flex-1">
                  ...
                  <Switch
                    ...
                    thumbColor={service.is_active ? "#FFFFFF" : "#6B7280"}
