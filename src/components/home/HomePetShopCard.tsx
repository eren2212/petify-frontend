...
export const HomePetShopCard = () => {
  ...
  const getPetShopImageUrl = (filename: string) => {
    ...
    return `${baseUrl}/home/images/shop-logo/${filename}`;
  };

  // Hepsini Gör butonuna tıklama
  const handleViewAll = () => {
    // TODO: Tüm pet shop'lar listesi sayfası oluşturulacak
    router.push("/(protected)/(tabs)/map");
  };

  // Kart tıklama - Pet Shop detay sayfasına git
  const handleCardPress = (id: string) => {
    router.push(`/(protected)/shops/${id}` as any);
  };
    ...
    <View className="py-2">
      ...
      <View className="px-5 gap-4">
        {petShops.map((petShop) => (
          <TouchableOpacity
            key={petShop.id}
            onPress={() => handleCardPress(petShop.id)}
            activeOpacity={0.8}
          ...
          >
            ...
            <View className="w-20 h-20 bg-gray-50 rounded-2xl p-1 shadow-inner border border-gray-50">
              {petShop.logo_url ? (
                <Image
                  source={{ uri: getPetShopImageUrl(petShop.logo_url) }}
