...
import { useUploadPetClinicLogo } from "@/hooks/useProfile";

interface PetClinicLogoPickerProps {
  currentLogoUrl?: string | null;
  className?: string;
}

export default function PetClinicLogoPicker({
  currentLogoUrl,
  className = "w-36 h-36",
}: PetClinicLogoPickerProps) {
  ...
  const getLogoUrl = () => {
    ...
    return `${process.env.EXPO_PUBLIC_API_URL}/petclinic/profile/logo/${currentLogoUrl}`;
  };
