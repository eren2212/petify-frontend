...
import { profileApi, petClinicApi } from "@/lib/api";
...
interface PetClinicEditProfileProps {
...
export default function PetClinicEditProfile({
  user,
}: PetClinicEditProfileProps) {
  const router = useRouter();
  const queryClient = useQueryClient();

  // Pet Shop profilini getir
  const { data: petClinicProfileResponse } = usePetClinicProfile();
  ...
  const [latitude, setLatitude] = useState(
    petClinicProfile?.latitude || 41.0082
  );
  const [longitude, setLongitude] = useState(
    petClinicProfile?.longitude || 28.9784
  );
  ...
  useEffect(() => {
    if (user?.profile && petClinicProfile) {
      ...
      if (petClinicProfile.working_hours) {
        parsedHours = petClinicProfile.working_hours.map((wh: any) => {
          if (wh.hours === "Kapalı") {
            return {
              day: wh.day,
              start: "09:00",
      ...
      reset({
        ...
        clinic_name: petClinicProfile.clinic_name || "",
        email: petClinicProfile.email || "",
        description: petClinicProfile.description || "",
        address: petClinicProfile.address || "",
        phone_number: petClinicProfile.phone_number || "",
        emergency_phone: petClinicProfile.emergency_phone || "",
        website_url: petClinicProfile.website_url || "",
        instagram_url: petClinicProfile.instagram_url || "",
        latitude: petClinicProfile.latitude || DEFAULT_LAT, // ✅ Eklendi
        longitude: petClinicProfile.longitude || DEFAULT_LNG,
        working_hours: parsedHours, // ✅ Form'a çalışma saatlerini ekliyoruz
      });

      setLatitude(petClinicProfile.latitude || 41.0082);
      setLongitude(petClinicProfile.longitude || 28.9784);
    }
  }, [user, petClinicProfile, reset]);
  ...
  const updatePetClinicMutation = useMutation({
    mutationFn: async (data: any) => {
      return await petClinicApi.updatePetClinicProfile(data);
    },
    onSuccess: () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await updatePetClinicMutation.mutateAsync(petClinicUpdateData);
      ...
      Alert.alert(
        ...
        error.response?.data?.message ||
          "Klinik bilgileri güncellenirken bir hata oluştu"
      );
    }
  };

  const isLoading =
    updateUserMutation.isPending || updatePetClinicMutation.isPending;
        ...
        <View className="bg-white rounded-2xl p-6 mb-4 shadow-sm">
          ...
          <View className="mb-4">
            ...
            <Controller
              ...
              name="phone_number"
              rules={{
                required: "Klinik telefonu zorunludur",
                pattern: {
                  value: /^[0-9]{10}$/,
                  message: "Geçerli bir 10 haneli telefon numarası girin",
                },
          ...
          <View className="mb-4">
            ...
            <Controller
              ...
              rules={{ required: "Acil telefon numarası zorunludur" }}
              render={({ field: { onChange, value } }) => (
                <TextInput
                  value={value}
                  onChangeText={onChange}
