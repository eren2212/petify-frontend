...
interface AddPetClinicProfileModalProps {
  visible: boolean;
  onClose: () => void;
  onSubmit: (profileData: PetClinicProfileData, logoUri: string | null) => void;
  isLoading?: boolean;
}

export interface PetClinicProfileData {
  clinic_name: string;
  description: string;
  address: string;
  latitude: number;
...
export default function AddPetClinicProfileModal({
  visible,
  onClose,
  onSubmit,
  isLoading = false,
}: AddPetClinicProfileModalProps) {
  // Form state
  ...
  const validateForm = (): boolean => {
    if (!clinicName.trim()) {
      Alert.alert("Hata", "Klinik adı zorunludur");
      return false;
    }
  ...
  const handleSubmit = () => {
    ...
    const profileData: PetClinicProfileData = {
            ...
            <View className="bg-white rounded-t-3xl h-5/6">
              ...
              <View className="px-6 py-4 border-t border-gray-200">
                <TouchableOpacity
                  ...
                  disabled={isLoading || !clinicName.trim() || !address.trim()}
                  className={`py-4 rounded-full ${
                    isLoading || !clinicName.trim() || !address.trim()
                      ? "bg-gray-300"
                      : "bg-primary"
                  }`}
                >
